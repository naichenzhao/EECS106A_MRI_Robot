...
def calclate_points(target_pnt, target_vec, curr_pnt, curr_vec):
    if in_circle(curr_pnt) :
        curr_line = Line(curr_pnt, curr_vec)
    else:
        curr_line = Line(curr_pnt, [curr_pnt[0]- ORIGIN[0], curr_pnt[1]- ORIGIN[1], curr_pnt[2]- ORIGIN[2]])
    ...
    # create path
    # len1, vec1 = get_path_circular(curr_inter, target_inter, origin = ORIGIN, res = 2)
    len1, vec1 = get_path_linear(target_inter, target_pnt, res = 2)

    # if vec2[0][0] > 0:
    #     vec2 = -1 * np.array(vec2)
            
    points = np.array(len1)
    vectors = np.array(vec1)
